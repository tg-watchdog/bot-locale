welcome_body =
    Merhaba! ben Telegram gruplarının bekçisiyim! 🐶
    Yeni üyeleri CAPTCHA ile kontrol eder, olası spam bot tehditlerini önlerim.
welcome_links_github = <a href="https://github.com/tg-watchdog/tg-watchdog">GitHub</a>
welcome_links_help = <a href="https://tgwatchdog.astrian.moe">Yardım</a>
welcome_links_community = <a href="https://t.me/tgwatchdog_chat">Topluluk</a>
welcome_links_channel = <a href="https://t.me/tgwatchdog_update">Kanal</a>
welcome_setmeasadmin = Beni grup yöneticisi olarak ayarla
verify_message = Merhaba! Ben { $groupname } grubunun bekçisiyim! 🐶
verify_loading = Bir saniye bekle⋯
verify_info = Gruba katılabilmek için aşağıdaki CAPTCHA doğrulama linkine tıklaman gerekiyor.
verify_btn = Doğrulamayı başlat
verify_btn_browser = Tarayıcı ile doğrula
verify_fallback_link = <i>Aşağıdaki düğme ile doğrulama başlatamazsanız, tarayıcıdan doğrulamak için <a href="{$fallback_link}">buraya tıklayın</a>.</i>
helpbot = Telegram Watchdog sizin desteklerinizle müthiş özellikler getirecek! <a href="https://t.me/tgwatchdog_update/32">Bağışlarınızla katkıda bulunun.</a>
